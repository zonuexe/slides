# Repository Guidelines

## Project Structure & Module Organization
- `docs/` hosts the VitePress site. Slide data flows from the per-slide files in `slides/` (`slides/${slug}.yaml`) through helpers in `lib/` (`generateSlidesData`, `loadSiteConfig`, `buildEventNarratives`, `groupEvent`/`buildEventGroups`), surfaced via `virtual:slides-data` and the page generators `docs/[slug]/index.paths.js` (per slide) and `docs/event/[group]/index.paths.js` (per event series).
- Year-specific directories such as `20150722esm/`, `20181124-vimconf/`, and `20191103-vimconf/` hold deck assets. Shared styling lives under `css/`, shared scripts in `js/`, and reusable templates in `deck/`.
- Binary artifacts and exports belong in `pdf/`, `zonuexe.png`, and the built static output under `docs/.vitepress/dist/`. Keep raw source files alongside their assets so automation can discover them.
- Python automation in `script/` orchestrates fetching static pages and updating metadata. Keep paths relative to repo root and do not hard-code user-specific locations.
- `slides/${slug}.yaml` is the hand-authored list metadata for one talk (title, date, download, events, tags, hashtags, related_articles). `file`/`meta`/`image` are derived from the slug as `pdf/${slug}.{pdf,yaml,png}`; set `stem:` only when the physical PDF basename differs from the slug (e.g. legacy `knNNNN_` files). The companion `pdf/${slug}.yaml` is machine-owned (extracted text/links/size, regenerated by `script/`); edit extracted fields manually only when the scraper needs correction. The list order is irrelevant — the catalog sorts by date descending at render time.
- YAML pitfall: any unquoted scalar containing a space-then-`#` (e.g. `name: PHP BLT #3`) is truncated at the comment marker, silently dropping everything after it. Always quote such values (`name: 'PHP BLT #3'`). A `#` with no preceding space (`Night#12`) is safe. Detect with `grep -rnE "name:.*[^'\"] #" slides/*.yaml`.

## Branch Strategy
- `master` receives new slide PDFs, the matching `slides/${slug}.yaml` metadata, companion `pdf/${slug}.yaml` updates, and any supporting VitePress or automation tweaks required to display them.
- `gh-pages` stores the VitePress build output from `npm run build` (i.e. `docs/.vitepress/dist`); commit the generated HTML and assets there so GitHub Pages can publish directly from that branch.

## Build, Test, and Development Commands
- `npm install` hydrates Node dependencies before any local run.
- `npm run dev` starts the VitePress dev server (base `/slides/`, defaults to `http://localhost:5173`); pass `--host --port` if you need a different origin.
- `/slides/local/` is **local mode**: a dev-only page that plays any uncommitted `*.pdf` sitting in the repository root, so a deck can be reviewed before it is published. See "Local Mode" below.
- `npm run build` emits the static site to `docs/.vitepress/dist`, and `npm run preview` serves that build for spot checks.
- `make fetch-static` runs `script/fetch_static_slides.py` against a running site (defaults to `http://localhost:5173`) to scrape pages into the specified output dir.
- `UV_CACHE_DIR=.uv-cache uv run python script/add_new_slides.py` processes staged PDFs, generates thumbnails, and writes `slides/${slug}.yaml`; append `--pdf pdf/new-talk.pdf` to target a specific file.
- Cross-block "泣き別れ" paragraphs left by the extractor's within-block merge are joined via the **`paragraph-merge` skill** (agent-judged, no LLM API call from code). `script/merge_paragraphs.py inventory` emits the candidate runs, the agent (or its subagents) decides which consecutive `para` fragments to merge, and `script/merge_paragraphs.py apply --decisions <file>` joins them deterministically; a per-page whitespace-stripped invariant refuses to write any page whose text would change. `... status` counts remaining runs. See `.claude/skills/paragraph-merge/SKILL.md`.
- `make script-format` and `make script-lint` invoke Ruff through `uvx` to keep Python helpers consistent.
- End-to-end PDF publishing steps are documented in `PDF_PUBLISH_SKILL.md`.

## Coding Style & Naming Conventions
- JavaScript uses ES modules, 2-space indentation, camelCase identifiers, and double-quoted strings. Group imports external → internal to match existing files.
- Prefer pure helpers inside `lib/`; keep request handlers thin so they remain easy to test manually.
- Python scripts should pass `ruff format` and `ruff check`; keep functions small and favor pathlib APIs already in use.
- When adding a new slide, drop the original PDF (Japanese filenames are fine) into the repo root and assign it a concise slug, then create `slides/${slug}.yaml`. Slugs start with a `YYYYMMDD_` prefix and continue with a descriptive kebab-case phrase (roughly 3–8 words, but add more if clarity demands). Ensure the slug reflects the talk’s topic succinctly rather than mirroring any English subtitle on the cover.

## Testing Guidelines
- There is no automated test suite; validate changes manually via `npm run dev`, refreshing slides that were touched.
- After modifying assets or YAML, run `npm run build` to ensure the static generation completes; use `npm run preview` or `make fetch-static` if you need to inspect the built output.
- To sanity-check YAML parsing without starting the site, run `node -e "import('./lib/slides.js').then(m => m.loadSlides())"`; it prints parse errors immediately.

## Commit & Pull Request Guidelines
- Match the existing history: short, imperative messages such as `Fix metadata` or `Add canonical`. Commit generated assets alongside their source changes.
- PRs should outline the affected slides, list any new automation commands used, and attach screenshots or PDFs when visual output changes.
- Link related issues or event pages where possible and call out large binary diffs so reviewers can prioritize source review.
- When generating HTML, use `target="_blank"` alone—`rel="noopener"` is no longer needed across the site.

## Slide Viewer Behaviour
- **List page** (`docs/index.md` + `SlidesCatalog.vue`): uses `generateSlidesData()` via `virtual:slides-data`; cards sort by date descending. `combinedContent` blends extracted PDF text, events, related articles, tags, hashtags (`#tag`), and download filenames; `events` are reduced to `{name, presented_at, location, place}` for display. Client-side data also hydrates from `docs/public/index.js` (`window.slidesData`).
- **Search** (`SlidesCatalog.vue` / `SlideCard.vue`): lazy-loads Fuse.js on demand with keys `title`, `slug`, `date`, `combinedContent`; query is mirrored to `?q=`. Snippets highlight matches and show events/tags only when present.
- **Detail page** (`docs/[slug]/index.paths.js` + `SlideDetailPage.vue`): builds head tags and descriptions from `buildEventNarratives` (falling back to the site description). OGP/Twitter and alternate links for PDF + oEmbed are emitted, the viewer iframe uses `siteConfig.embed` to point at the PDF mirror, and `window.slideConfig` drives `docs/public/js/slide-functions.js` for sizing, fullscreen, sharing, and image download/copy controls. PDF text/links render from `pdfMeta` with page ordering preserved.
- **Speaker view button** (`slide-pdf.js` embed): desktop-only; stays visible on page 1 and hides on later pages, opening `/slide-pdf.js/speaker.html` with the mirrored PDF for presenters.

## Local Mode
- `docs/.vitepress/plugins/local-mode.js` mounts `/slides/local/` **in `configureServer` only**, so nothing about it reaches `npm run build`; uncommitted local files must never leak into the published site.
- Search paths are configured, defaulting to `~/Documents` and `.` (the repository root — the publishing inbox, see "Coding Style & Naming Conventions"). `lib/local-config.js` resolves them: `local.yaml` (gitignored, per machine) has the last word via its `paths:` key, otherwise `_site.yaml`'s `local.paths`, otherwise the built-in defaults. `~` means home, relative entries resolve against the repository root, duplicates collapse, and missing directories are skipped silently. The config is re-read per request, so editing `local.yaml` only needs a page reload.
- Because `_site.yaml` also feeds the published `siteConfig`, `slides-data.js` strips the `local` key before serializing it into `virtual:slides-data` — machine-local paths have no business in the public bundle.
- Only the top level of each directory is scanned; `~/Documents` should not cost a recursive walk. Decks are listed newest first with the source path shown. Directories inside the repository additionally drop anything committed and unmodified, so this previews a deck before `add_new_slides.py` has run and before any `slides/*.yaml` exists, while `pdf/` stays out of the way. Without git nothing is filtered.
- Each deck is addressed by a short hash of its absolute path (`/slides/local/<id>/`, PDF at `/slides/local/pdf/<id>.pdf`). Filenames collide across directories, and routing through an id table means a request path never reaches the filesystem.
- **The production embed cannot be reused here.** `https://zonuexe.github.io/slide-pdf.js/` cannot fetch a `http://localhost` PDF: the browser blocks the request outright (`ERR_BLOCKED_BY_CLIENT`) before it reaches the server, and neither CORS nor Private Network Access headers change that. The viewer therefore has to be served from the dev origin, at `/slides/local/viewer/`.
- That viewer comes from the sibling `../slide-pdf.js` checkout when one exists (works offline, and lets viewer changes be tried against real decks); otherwise the plugin proxies `embed.base_url` from `_site.yaml`. `slide-pdf.js` resolves `app.js`, `pdf.worker.mjs`, `cmaps/`, and `speaker.html` relative to its own URL, so mounting the directory as-is is enough.
- The page is plain server-rendered HTML rather than a VitePress route, which is what keeps it out of the build. It is a player, not a preview of the published detail page: a "全画面で再生" button and the deck's own controls, no download link, no extracted text (that would need the Python extractor, which does not belong in the dev server).
- Page count and page size come from shelling out, in order: `pdfinfo` (poppler — gives both at once), `mdls -name kMDItemNumberOfPages` (always present on macOS but returns `(null)` for unindexed or unreadable files), then `gs ... pdfpagecount`. Size falls back to a `/MediaBox` scan of the raw bytes and finally to 16:9; page count is simply omitted when nothing can determine it, rather than guessed. Results are cached on `path:size:mtime`, so a replaced PDF is re-inspected automatically.
- Fullscreen targets the viewer `<iframe>` itself, so `slide-pdf.js` picks up the resize and repaints at full size with no extra layout work here. **Do not gate anything on the promise `requestFullscreen()` returns** — in embedded browser views that never get a fullscreen surface it neither resolves nor rejects; `fullscreenchange` is the only reliable signal, and that is where focus is moved into the iframe so the arrow keys keep working.

## Event Grouping & Series Pages
- `lib/event-groups.js` derives an event "series" from each `event.name` heuristically (pure function `groupEvent(name) -> {id, name, isSeries}`, plus `buildEventGroups(slides)` for aggregation). `slides/*.yaml` is never mutated for grouping — the series is computed at build time. Cases the rules cannot resolve are pinned in the `OVERRIDES` map; a new regional/series brand is usually a one-line addition to `PHP_CONFERENCE_REGIONS` or `SERIES_RULES`.
- A talk can belong to more than one series via `groupEvents(name)` (the plural form `buildEventGroups`/`SlideDetailPage` use): the `CO_MEMBERSHIPS` map adds co-located series so an event-within-event shows up under both — e.g. the `LLイベント` family (`Learn Languages 20XX`) is also listed under `ODC` because it runs as a co-located track there.
- Key rules: `PHPカンファレンス20XX` (no region) collapses into `PHP Conference Japan`; regional editions collapse into `PHPカンファレンス<region>` (year, 前夜祭/懇親会/リジェクトコン, and (非公式) prefixes are folded in). Other recurring events (`PHPerKaigi`, `PHP勉強会＠東京`, `PHP BLT`, `LLイベント` = the `Learn Languages 20XX` family, `TechFeed Experts Night`, `pixiv BOOT CAMP`) are series-ified too. Single talks stay as their own `solo:` group (`isSeries:false`) and get no page.
- **Series pages** (`docs/event/[group]/index.paths.js` + `EventGroupPage.vue`): one `/event/<id>/` page per `isSeries` group, reusing `SlideCard` for the talk list (date desc) and emitting OGP/canonical like slide pages. **Series index** (`docs/event/index.md` + `EventGroupsIndex.vue`) lists series at `/event/` and is linked from the global nav ("Events").
- **Navigation into series pages**: `SlideCard` links each series-event name to its `/event/<id>/` page; `SlideDetailPage` shows an "イベントシリーズ:" link block; `sitemap.js` also enumerates the series pages. All of these import `groupEvent`/`buildEventGroups` directly (the module is browser-safe, no Node deps).
